...
if __name__ == '__main__':
    ...
    messages = [
        ...
        HumanMessage("I lives in Bhopal."),
    ]

    response = model.invoke(messages)
    print(response)              # Prints the full response object (includes metadata)
    # print(response.pretty_print())  # This to format the response nicely (if supported)
    print(response.content)      # Prints just the translated text
